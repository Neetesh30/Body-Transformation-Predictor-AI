...
class DoctorController extends Controller
{
     ...
     public function store__V2_Working_AI_Image(Request $request)
    {
     ...
     public function store_complete_master_working(Request $request)
    {
        ...
        $request->validate([
            ...
            'phone' => [
                'required',
                'regex:/^[6-9]\d{9}$/', // Valid Indian mobile number pattern
            ],
            'height_feet' => 'required|numeric|min:1|max:8',
            'height_inches' => 'required|numeric|min:0|max:11',
            'weight' => 'required|numeric|min:10|max:300',
            'diet' => 'required|in:Veg,Non-Veg,Vegan',
            'lifestyle' => 'required|in:Sedentary,Moderate,Active',
            'image' => 'nullable|image|mimes:jpeg,png,jpg,webp|max:2048', // 2 MB limit
        ], [
   ...
   $client = new Client();
    $apiKey = '29fdab8e58f926b21b90e6883a929391820590b35438d9ab99abb6f2ee561a39'; // better to store in .env
    ...
    $response = $client->post('https://wasenderapi.com/api/send-message', [
        ...
        'json' => [
            'to' => '+919768300737',
            'text' => 'Here is your BMI report',
            'documentUrl' => asset(Storage::url($pdfFileName)),
        ...
        if ($request->hasFile('image')) {
            // Store uploaded image
                ...
                $user = User::create([
                    ...
                    'image' => $uploadedImagePath, // store path relative to storage/app/public
                ]);
